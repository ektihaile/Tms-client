...
type HasKnownRecordMember<T> = true extends (T extends unknown ? IsKnownRecord<T> : never) ? true : false;
type NonRecordMembers<T> = T extends unknown ? IsKnownRecord<T> extends true ? never : T : never;
type OmitPrivate<T> = {
    [K in keyof T as K extends `_${string}` ? never : K]: T[K];
};

type DeepSignal<T> = Signal<T> & (IsKnownRecord<T> extends true ? Readonly<{
    [K in keyof T]: DeepSignalOf<T[K]>;
}> : unknown);
type DeepSignalOf<T> = HasKnownRecordMember<T> extends true ? DeepSignalRecordMembers<T> | DeepSignalNonRecordMembers<T> : Signal<T>;
type DeepSignalRecordMembers<T> = T extends unknown ? IsKnownRecord<T> extends true ? DeepSignal<T> : never : never;
type DeepSignalNonRecordMembers<T> = [NonRecordMembers<T>] extends [never] ? never : Signal<NonRecordMembers<T>>;
 ...
 * ```
 */
declare function deepComputed<T>(computation: () => T): DeepSignalOf<T>;
...
export type { DeepSignal, DeepSignalOf, EmptyFeatureResult, PartialStateUpdater, Prettify, SignalMethod, SignalState, SignalStoreFeature, SignalStoreFeatureResult, SignalStoreFeatureType, StateSignals, StateSource, StateWatcher, WritableStateSource };
